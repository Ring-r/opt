...
namespace PolygonPlacingTest
{
    public class PlacingOpt
    {
        ...
        public PlacingOpt(PolygonWithMinMax[] polygons, Vector strip_vector)
        {
            ...
            do
            {
                ...
                for (int i = 0; i < polygons.Length - 1; i++)
                    for (int j = i + 1; j < polygons.Length; j++)
                    {
                        ...
                        //PlaneDividing pd = new PlaneDividing(new Polygon.Iterator(i, polygons[i], 0), new Polygon.Iterator(j, polygons[j], 0));
                        //pd.Find();

                        #region Временный код.
                        PlaneDividing pd;
                        Polygon.Iterator it1i = new Polygon.Iterator(i, polygons[i], 0);
                        Polygon.Iterator it1j = new Polygon.Iterator(j, polygons[j], 0);
                        double ed1 = Find(it1i, it1j);

                        Polygon.Iterator it2i = new Polygon.Iterator(i, polygons[i], 0);
                        Polygon.Iterator it2j = new Polygon.Iterator(j, polygons[j], 0);
                        ...
                        Vector vector = pd.IteratorPlane.Point(1) - pd.IteratorPlane.Point(0);
                        Vector normal = new Vector { X = -vector.Y, Y = vector.X };
        ...
        private double Find(Polygon.Iterator iti, Polygon.Iterator itj)
        {
            int gi = -1;
            int gj = -1;
            ...
            for (int i = 0; i < iti.Count; i++)
            {
                ...
                {
                    double ed = Opt.Geometrics.Extentions.PlaneExt.Расширенное_расстояние(iti.Plane(i), itj.Point(j));
                    if (led > ed)
                    {
                        led = ed;
                        lj = j;
        ...
        public System.Collections.Generic.List<Plane> GetPlanes(int index)
        {
            System.Collections.Generic.List<Plane> res = new System.Collections.Generic.List<Plane>();
            ...
            for(int i=0; i<polygons.Length-1;i++)
                for (int j = i + 1; j < polygons.Length; j++)
                {
                    if (index == i || index == j)
                    {
                        Polygon.Iterator iti = new Polygon.Iterator((int)Gs[k][0], polygons[(int)Gs[k][0]], (int)Gs[k][2]);
                        res.Add(iti.Plane(0));
                    }
                    k++;
                }
