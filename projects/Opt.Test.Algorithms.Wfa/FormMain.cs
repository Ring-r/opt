...
namespace Opt.Algorithms.WFAT
{
    public partial class FormMain : Form
    {
        ...
        private void miLoad_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < n; i++)
                circles.Add(new Circle { Pole = new Point2d { X = double.NegativeInfinity, Y = 0 }, Scalar = double.Parse(sr.ReadLine()) });
        ...
        private void FormPlaceCircle_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (int i = 0; i < circles.Count; i++)
                if (0 <= circles[i].Pole.X + circles[i].Scalar && circles[i].Pole.X - circles[i].Scalar < ClientSize.Width)
                {
            ...
            for (int i = 0; i < circles.Count && x < ClientSize.Width; i++)
            {
                ...
                {
                    e.Graphics.DrawEllipse(System.Drawing.Pens.Black, (float)x, (float)y, 2 * (float)circles[i].Scalar, 2 * (float)circles[i].Scalar);
                    e.Graphics.FillEllipse(System.Drawing.Brushes.Silver, (float)x, (float)y, 2 * (float)circles[i].Scalar, 2 * (float)circles[i].Scalar);
                    x += 2 * circles[i].Scalar;
                }
            }
            #endregion
                ...
                if (vertex != null)
                {
                    ...
                    for (int i = 0; i < triples.Count; i++)
                    {
                        ...
                        do
                        {
                            ...
                            //        //if (Circle2dExt.Расширенное_расстояние(circle, circle_next) < 1e-3)
    ...
    public static class DrawExtention
    {
        public static void FillAndDraw(this Circle circle, System.Drawing.Graphics graphics, System.Drawing.Color color_brush, System.Drawing.Color color_pen)
        {
            if (circle != null)
            {
                graphics.FillEllipse(new System.Drawing.SolidBrush(color_brush), (float)(circle.Pole.X - circle.Scalar), (float)(circle.Pole.Y - circle.Scalar), (float)(2 * circle.Scalar), (float)(2 * circle.Scalar));
                graphics.DrawEllipse(new System.Drawing.Pen(color_pen), (float)(circle.Pole.X - circle.Scalar), (float)(circle.Pole.Y - circle.Scalar), (float)(2 * circle.Scalar), (float)(2 * circle.Scalar));
