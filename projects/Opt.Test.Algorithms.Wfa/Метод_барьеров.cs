    ...
    public class PlacingAll : Placing
    {
        ...
        protected double ValueG(double[] G, double[] X)
        {
            ...
            if (j >= 0)
            {
                ...
                double r = circles[i].Scalar + circles[j].Scalar - 2 * e;
                //return x * x + y * y - r * r;
        ...
        public override void CalculateStart()
        {
            ...
            for (int i = 0; i < circles.Length; i++)
            {
                ...
                Gs[k][1] = -3;
                Gs[k][3] = -1;
                Gs[k][5] = -circles[i].Scalar + height;
                k++;
                #endregion
    ...
    public class PlacingOpt : Placing
    {
        ...
        protected override double ValueGsWithCheck(double[] X)
        {
            ...
            for (int i = 0; i < circles.Length; i++)
            {
                ...
                value = X[2 * i] - circles[i].Scalar + e;
                if (value > 0)
                    res += 1 / value;
                else
                    return double.PositiveInfinity;
                ...
                value = X[2 * i + 1] - circles[i].Scalar + e;
                if (value > 0)
                    res += 1 / value;
                else
                    return double.PositiveInfinity;
                ...
                value = -X[2 * i + 1] - circles[i].Scalar + height + e;
                if (value > 0)
                    res += 1 / value;
                else
                    return double.PositiveInfinity;
                ...
                value = -X[2 * i] - circles[i].Scalar + X[index_strip] + e;
                if (value > 0)
                    res += 1 / value;
                else
                    return double.PositiveInfinity;
                ...
                for (int k = 0; k < 3; k++)
                {
                    if (vertex.Cros.Somes.LastChecked != data)
                    {
                        ...
                        if ((vertex.Next.DataInVertex is Circle) && (vertex.Prev.DataInVertex is Circle))
                        {
                            ...
                            double r = circles[j].Scalar + circles[i].Scalar - 2 * e;
                            value = Math.Sqrt(x * x + y * y) - r;
                            if (value > 0)
                                res += 1 / value;
                            else
                    ...
                    if (length > eps)
                    {
                        ...
                        #region Проверяем и переразбиваем модель вокруг вершины, связанной со вставленным объектом.
                        {
                            for (int i = 0; i < triples.Count; i++)
                            {
                                ...
                                do
                                {
                                    while (Geometric2dExt.Расширенное_расстояние(vertex_temp.DataInVertex, vertex_temp.Cros.Somes.CircleDelone) < 0)
                                    {
                                        vertex_temp.Rebuild();

                                        vertex_temp.SetCircleDelone(Geometric2dExt.Круг_Делоне(vertex_temp.Prev.DataInVertex, vertex_temp.DataInVertex, vertex_temp.Next.DataInVertex));
                                        vertex_temp.Next.Cros.SetCircleDelone(Geometric2dExt.Круг_Делоне(vertex_temp.Next.Cros.Prev.DataInVertex, vertex_temp.Next.Cros.DataInVertex, vertex_temp.Next.Cros.Next.DataInVertex));
                                    }
