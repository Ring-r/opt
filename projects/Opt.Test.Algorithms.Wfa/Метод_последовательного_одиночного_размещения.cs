    ...
    public class PlacingWithCloseModel : Opt.Algorithms.Placing, Opt.Algorithms.IWithClosenessModel
    {
        ...
        private bool Функция_расширенного_расстояния_на_отрезке_монотонна(Vertex<Geometric2d> vertex)
        {
            if (vertex.Next.DataInVertex is Plane2d && vertex.Prev.DataInVertex is Plane2d)
                return true;
            Plane2d plane = Geometric2dExt.Серединная_полуплоскость(vertex.Next.DataInVertex, vertex.Prev.DataInVertex);
            return Plane2dExt.Расширенное_расстояние(plane, vertex.Somes.CircleDelone.Pole) * Plane2dExt.Расширенное_расстояние(plane, vertex.Cros.Somes.CircleDelone.Pole) > 0;
        }
