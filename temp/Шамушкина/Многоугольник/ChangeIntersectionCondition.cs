...
{
    class ChangeIntersectionCondition
    {
        public int[] NewCond; //хранит разделяющую линию для пары выпуклых многоугольников

        public ChangeIntersectionCondition(int[] cond, Polygon[] polygons)
        {
            NewCond = new int[2];
        ...
        public int[] DividingLine(int[] cond, Polygon[] polygons)
        {
            ...
            for (int i = 0; i < pol1.Count; i++)
            {
                if (i != cond[2])
                {
                    ...
                    a = x2[1] - x1[1];
                    b = -x2[0] + x1[0];
                    c = -(polygons[cond[0]].polus[0] + x1[0]) * (x2[1] - x1[1]) + (x2[0] - x1[0]) * (polygons[cond[0]].polus[1] + x1[1]);
